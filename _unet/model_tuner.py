...
def build_model(hp):
    ...
    model.compile(total_loss, metrics)
    return model
...
for weight in weights:
    ...
    image_mode = 'crop'
    add_info = f'binary_{curr_class}_w_true_weight_{current_weights[0]}_optimalized'
    model_name = f'unet_{model_num}_{BACKBONE}_{loss}_{image_size}_size_{image_mode}_{5}_epoch_{add_info}'
    ...
    tuner.search(train_dataloader, epochs = 5, validation_data = valid_dataloader)
    ...
    print(tuner.get_best_hyperparameters(), file = f)
