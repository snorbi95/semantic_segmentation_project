...
class Dataset:
    ...
    def __getitem__(self, i):
        ...
        # read data
        full_image = cv2.imread(self.images_fps[i], cv2.IMREAD_COLOR)
        full_image = cv2.resize(full_image, (full_img_size[0], full_img_size[1]), interpolation=cv2.INTER_NEAREST)
        full_mask = cv2.imread(self.masks_fps[i], cv2.IMREAD_GRAYSCALE)
        full_mask = cv2.resize(full_mask, (full_img_size[0], full_img_size[1]), interpolation=cv2.INTER_NEAREST)
        ...
        while np.sum(mask) < full_mask_len * 0.2:
            random_x = np.random.randint(0, full_image.shape[0] - img_size[0])
            random_y = np.random.randint(0, full_image.shape[1] - img_size[1])
            mask = full_mask[random_x:random_x + img_size[0], random_y:random_y + img_size[1]]
...
class Dataloder(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, i):
        ...
        for j in range(batch_size):
            data.append(self.dataset[i % train_len])
...
dice_loss = losses.dice_coef_loss#sm.losses.DiceLoss()
...
train_dataloader = Dataloder(train_dataset, batch_size=BATCH_SIZE, shuffle=True, length=train_len)
