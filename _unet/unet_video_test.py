...
class Dataset:
    ...
    def __init__(
        ...
        self.ids.sort(key = lambda x: int(x.split('.')[0].split('_')[-1]))
        self.images_fps = [os.path.join(images_dir, image_id) for image_id in self.ids]
        self.images_fps.sort(key = lambda x: int(x.split('.p')[0].split('_')[-1]))
        if masks_dir:
            self.masks_fps = [os.path.join(masks_dir, image_id) for image_id in self.ids]
            self.masks_fps.sort(key=lambda x: int(x.split('.p')[0].split('_')[-1]))
        else:
            self.masks_fps = None
    ...
    def __getitem__(self, i):
        ...
        image = cv2.imdecode(np.fromfile(self.images_fps[i], dtype=np.uint8), cv2.IMREAD_UNCHANGED)
...
ureter_model = keras.models.load_model(f'models/binary_ureter/equal_dataset_distribution/unet_1_efficientnetb3_weighted_jaccard_loss_8_batch_size_256_size_crop_20_epoch_binary_ureter_0.4_true_weight_equal_set_#3.h5',
...
arthery_model = keras.models.load_model(f'models/binary_arthery/equal_dataset_distribution/unet_1_efficientnetb3_weighted_jaccard_loss_8_batch_size_256_size_crop_20_epoch_binary_arthery_0.45_true_weight_equal_set_#4.h5',
                                custom_objects={'jaccard_loss': jaccard_loss, 'jaccard_score_true_class': jaccard_score_true_class, 'jaccard_score_all': jaccard_score_all,
                                                'dice_coef': dice_coef})
...
test_dir = f'../test_videos/balra erek jobb szélső ureter'
test_frame_names = [img for img in os.listdir(test_dir) if not os.path.isdir(img)]
test_frame_names.sort(key = lambda x: int(x.split('.')[0].split('_')[-1]))
...
for n in range(len(test_dataset)):
    ...
    test_frame_original = cv2.resize(test_frame_original, (img_size[0],img_size[1]))
    reg_pred_2 = map_values_to_rgb(reg_pred_2)
    reg_pred_2 = cv2.resize(reg_pred_2, (img_size[0], img_size[1]))
    fused_image = Image.blend(Image.fromarray(test_frame_original), Image.fromarray(reg_pred_2.astype(np.uint8)), 0.35)
    try:
        io.imsave(f'{test_dir}/predictions/pred_frame_{n}.png',np.array(fused_image))
    except:
        os.mkdir(f'{test_dir}/predictions')
        io.imsave(f'{test_dir}/predictions/pred_frame_{n}.png',np.array(fused_image))
    # fig, ax = plt.subplots(1,3)
    # ax[0].imshow(test_frame_original)
    # ax[1].imshow(reg_pred_2)
    # fused_image = Image.blend(Image.fromarray(test_frame_original), Image.fromarray(reg_pred_2.astype(np.uint8)), 0.5)
...
# video_name = f'{test_dir}/prediction_video.avi'
#
# video = cv2.VideoWriter(video_name,0,1,(model_img_size[0], model_img_size[1]))
#
# for image in images:
